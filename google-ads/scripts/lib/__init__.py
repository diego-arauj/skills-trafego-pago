...
# macOS: o resolver c-ares do gRPC trava em algumas configurações de rede.
# Forçar resolver nativo resolve o hang em get_service().
...
_GLOBAL_SKILL_DIR = os.path.expanduser("~/.claude/skills/google-ads")


def _env_filename():
    """Retorna o nome do arquivo .env baseado em GOOGLE_ADS_ENV (ex: 'clientex' -> '.env.clientex')."""
    profile = os.environ.get("GOOGLE_ADS_ENV", "").strip()
    return f".env.{profile}" if profile else ".env"


def _load_env_file():
    """Carrega variaveis de um .env (ou .env.<perfil>) sem precisar de source no zshrc."""
    env_file = _env_filename()
    search_paths = [
        os.path.join(_GLOBAL_SKILL_DIR, env_file),
        os.path.join(_SKILL_DIR, env_file),
    ]
...
def _find_yaml_path():
    """Encontra google-ads.yaml (ou google-ads-<perfil>.yaml) para fallback do SDK."""
    profile = os.environ.get("GOOGLE_ADS_ENV", "").strip()
    yaml_name = f"google-ads-{profile}.yaml" if profile else "google-ads.yaml"
    search_paths = [
        os.path.join(_GLOBAL_SKILL_DIR, yaml_name),
        os.path.join(_SKILL_DIR, yaml_name),
    ]
...
# Deadline (segundos) para toda chamada RPC. Sem isso, o stream do gRPC
# (grpcio >= 1.80 no macOS) pode congelar indefinidamente em service.search().
# Configuravel via GOOGLE_ADS_RPC_TIMEOUT.
RPC_TIMEOUT = float(os.environ.get("GOOGLE_ADS_RPC_TIMEOUT", "120"))
...
def run_query(customer_id, query):
    ...
    results = []
    try:
        response = service.search(customer_id=customer_id, query=query, timeout=RPC_TIMEOUT)
        for row in response:
