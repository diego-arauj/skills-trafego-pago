...
def _env_path():
    """Retorna o caminho do .env ativo, respeitando GOOGLE_ADS_ENV."""
    profile = os.environ.get("GOOGLE_ADS_ENV", "").strip()
    filename = f".env.{profile}" if profile else ".env"
    return os.path.join(SKILL_DIR, filename)


ENV_PATH = os.path.join(SKILL_DIR, ".env")  # fallback para cmd_check (pre-_load_env)
SERVER = "127.0.0.1"
...
def run_oauth():
    ...
    _extra = {}
    _hint = os.environ.get("GOOGLE_ADS_OAUTH_LOGIN_HINT", "").strip()
    if _hint:
        _extra["login_hint"] = _hint
    _authuser = os.environ.get("GOOGLE_ADS_OAUTH_AUTHUSER", "").strip()
    if _authuser:
        _extra["authuser"] = _authuser
    ...
    # OAUTHLIB_RELAX_TOKEN_SCOPE permite que o Google devolva scopes extras sem erro
...
def save_refresh_token(token):
    """Salva o refresh token no .env ativo (respeita GOOGLE_ADS_ENV)."""
    target = _env_path()
    if not os.path.isfile(target):
        print(f"ERRO: arquivo nao encontrado em {target}")
        sys.exit(1)
