...
_CONTAS_PATH = os.path.join(_SKILL_DIR, "contas.yaml")

def _load_contas():
    """Carrega contas.yaml. Retorna dict {slug: {...}} ou {} se não existir."""
    if not os.path.isfile(_CONTAS_PATH):
        return {}
    try:
        with open(_CONTAS_PATH, "r", encoding="utf-8") as f:
...
def _find_client_token(account_id):
    """Retorna token do cliente que possui account_id, ou None se não definido."""
    if not account_id:
        return None
    contas = _load_contas()
    normalized = account_id if account_id.startswith("act_") else f"act_{account_id}"
    for _slug, info in contas.items():
        if not isinstance(info, dict):
            continue
        acct = info.get("conta_anuncio", "")
        if not acct.startswith("act_"):
            acct = f"act_{acct}"
        if acct == normalized and info.get("token"):
            return info["token"]
    return None
...
def init_api(token=None):
    """Inicializa o SDK. Se token for passado, usa ele. Caso contrário, lê do .env."""
    global _api_initialized, _current_token
    ...
    if token:
        ...
        print(f"Token do cliente ({mask_token(token)})", file=sys.stderr)
        return
...
def resolve_account(args_account=None):
    """Resolve account ID e troca token se o cliente tiver um próprio em contas.yaml."""
    if args_account:
        acct = args_account
        if not acct.startswith("act_"):
            acct = f"act_{acct}"
    else:
        acct = get_default_account_id()

    client_token = _find_client_token(acct)
    if client_token:
        init_api(token=client_token)
