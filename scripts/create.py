...
def cmd_image(args):
    ...
    if args.file:
        ...
        if not os.path.isfile(path):
            print_error(f"Arquivo nao encontrado: {path}")
            return
    else:
    ...
    try:
        # O SDK novo nao aceita mais AdAccount.create_ad_image(files=...).
        # Caminho estavel: AdImage.remote_create() com o campo `filename`.
        image = AdImage(parent_id=account_id)
        image[AdImage.Field.filename] = path
        if args.name:
            image[AdImage.Field.name] = args.name
        image.remote_create()
        safe_delay(1)

        result = {'hash': image[AdImage.Field.hash], 'url': image.get(AdImage.Field.url)}
        print("Imagem enviada com sucesso", file=sys.stderr)
        print_json(result)
    finally:
        if tmp is not None:
            os.unlink(tmp.name)
...
def cmd_video(args):
    ...
    if args.file:
        ...
        video = AdVideo(parent_id=account.get_id_assured(), api=account.get_api_assured())
        video[AdVideo.Field.filepath] = args.file
        result = VideoUploader().upload(video)
    else:
...
def main():
    ...
    g.add_argument("--url", help="URL publica da imagem para upload")
    g.add_argument("--file", help="Caminho local da imagem para upload")
    ...
    source.add_argument("--url", help="URL publica do video para upload")
    source.add_argument("--file", help="Caminho local do video para upload")
