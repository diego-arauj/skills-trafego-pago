...
def _page(cursor, limit=None):
    """Collect the current page and preserve cursors for incremental browsing."""
    results = [cursor[i] for i in range(len(cursor))]
    if limit:
        results = results[:limit]
    paging = getattr(cursor, "paging", None) or getattr(cursor, "_paging", None)
    if not paging:
        after = getattr(cursor, "params", {}).get("after")
        paging = {
            "cursors": {"after": after} if after else {},
            "has_next": not getattr(cursor, "_finished_iteration", True),
        }
    return {"data": results, "paging": paging or {}}
...
def cmd_images(args):
    ...
    if args.with_paging:
        print_json(_page(images, args.limit))
    else:
        print_json(_collect(images, args.limit, auto_paginate=not args.first_page))
...
def cmd_videos(args):
    ...
    if args.with_paging:
        print_json(_page(videos, args.limit))
    else:
        print_json(_collect(videos, args.limit, auto_paginate=not args.first_page))
...
def build_parser():
    ...
    p.add_argument("--first-page", action="store_true", help="Do not auto-paginate")
    p.add_argument("--with-paging", action="store_true", help="Include pagination cursors")
