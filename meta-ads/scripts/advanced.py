...
def _copy_creative_with_url_tags(account_id, source_creative_id, new_url_tags):
    ...
    fields = [
        ...
        "link_url", "title", "body",
        "effective_object_story_id", "degrees_of_freedom_spec",
    ]
    ...
    # Criativo feito a partir de post existente (UI / dark post ja publicado):
    # o object_story_spec volta so com page_id + instagram_user_id, sem link_data
    # nem video_data. Recriar com esse spec truncado da erro #3 ("Application does
    # not have the capability"). O caminho certo e referenciar o post original.
    oss = data.get("object_story_spec") or {}
    has_story_content = any(
        k in oss for k in ("link_data", "video_data", "photo_data", "text_data")
    )
    post_id = data.get("effective_object_story_id")

    if post_id and not has_story_content:
        params["object_story_id"] = post_id
        if data.get("degrees_of_freedom_spec"):
            params["degrees_of_freedom_spec"] = data["degrees_of_freedom_spec"]
    else:
